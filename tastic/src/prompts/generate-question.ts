...
export function buildGenerateQuestionPrompt(input: QuestionPromptInput): { system: string; user: string } {
  ...
  const system = `너는 문화 콘텐츠 감상 인터뷰어다. 사용자가 감상한 작품에 대해 자연스럽게 감상을 끌어내는 것이 목표다.
...
반드시 아래 JSON 형식으로만 응답하라:
{
  ...
  const conversationText =
    conversationHistory.length > 0
      ? conversationHistory
          .map((entry) =>
            `${entry.role === "interviewer" ? "인터뷰어" : "사용자"}: ${entry.text}`
          )
          .join("\n")
      : "(이전 대화 없음)";
